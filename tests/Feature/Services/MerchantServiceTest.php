...
class MerchantServiceTest extends TestCase
{
    ...
    public function test_create_merchant()
    {
        ...
        $this->assertEquals(User::TYPE_MERCHANT, $user->type);
        $this->assertTrue(Hash::check($data['api_key'], $user->password));
    }
    ...
    {
        /** @var Merchant $merchant */
        $merchant = Merchant::factory()->for(User::factory())->create();
        $affiliate = Affiliate::factory()->for($merchant)->for(User::factory())->create();
        $orders = Order::factory()->for($merchant)->for($affiliate)->count(10)->create();
        $paid = $orders->random();
        $paid->update([
